---
name: zk-llm-search
description: Use when the user asks to find, search, or look up a note in their Zettelkasten with specific named terms — "find my note about X", "search for Y in my notes", "list everything tagged Z". For fuzzy or vibe queries, use zk-llm-associative-recall instead. For exploring a topic across many notes, use zk-llm-deep-research.
---

# Searching a Zettelkasten with zk-llm

`zk-llm` is a literal, ripgrep-backed retrieval CLI over the user's Zettelkasten archive. It does not do semantic similarity — what you type is what it searches. For fuzzy matches, delegate to `zk-llm-associative-recall`.

## The three subcommands

- `zk-llm search` — AND-combined predicates, YAML list output.
- `zk-llm tag NAME` — shortcut for `search --tag NAME`.
- `zk-llm show REF [REF ...]` — print frontmatter + raw body per ref.

If no archive is configured, the CLI tells you how: pass `--archive PATH` on the subcommand, set `ZK_LLM_ARCHIVE`, or write `archive: /path` to `~/.config/zk-llm/config.yaml`.
...
|---|---|
| Notes with a specific hashtag | `zk-llm tag NAME` |
| Notes with two hashtags at once | `zk-llm search --tag A --tag B` |
| Notes mentioning a distinctive single word | `zk-llm search --word TERM` |
| Notes containing a specific phrase | `zk-llm search --phrase "TEXT"` or `zk-llm search "TEXT"` |
| Notes about a topic without knowing exact terms | Delegate to `zk-llm-associative-recall`. |
| Explore around a starting note | Delegate to `zk-llm-deep-research`. |
...
- Combine predicates (AND) to narrow: `zk-llm search --tag learning --word inversion`.
...
- Don't use this skill for fuzzy recall ("that thing I wrote about..."). Delegate to `zk-llm-associative-recall`.
